...
import photo from "../assets/icons/photo.svg?raw";
...
addCollection({
  ...
  icons: {
    ...
    chat: { body: svgBody(chat) },
    photo: { body: svgBody(photo) },
    send: { body: svgBody(send) },
    stop: { body: svgBody(stop) },
    trash: { body: svgBody(trash) },
    close: { body: svgBody(close) },
  },
});
